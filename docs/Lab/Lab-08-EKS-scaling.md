# Lab 05B — EKS Scaling and Service Exposure

*Practical 4, Part B — making the cluster grow, and letting the world in*

!!! info "Numbering — the same note as Part A"
    ...
    Practical 4  ->  Lab 05A, Lab 05B            (EKS)   <- descriptor Practical 3
...
Lab 05A left the USMS application in a deliberately awkward state. Three microservices, all healthy,
...
This laboratory answers the two questions Part A left open, and they turn out to be one question
asked twice:
...
**scaling is one integer**. Lab 04C established that: Application Auto Scaling's entire job was to
...
```text
Lab 04C   Application Auto Scaling   an AWS service   OUTSIDE your workload   writes desiredCount
Lab 05B   HorizontalPodAutoscaler    a controller     INSIDE your cluster     writes replicas
```
...
Lab 05A  EKS ................. cluster, node group, three microservices, service discovery
Lab 05B  EKS scaling and exposure   THIS LAB
Lab 06   S3 .................. the bucket that three IAM policies already name
Lab 07   Lambda .............. functions triggered from that bucket
```

!!! warning "Two things from Lab 05A must be true before you start"
    **`usms-enrolment` must declare a CPU request.** Lab 05A Step 13 set `50m`. The HPA computes
    ...
    **The cluster must carry the `_lb_ports_` tag.** Lab 05A Step 8 set it to `8081,8082`. Without
...
10. **Compare** Kubernetes Ingress with the Application Load Balancer of Lab 04B, in both directions,
...
- **Lab 05A**, complete, with `configs/lab-05a.env` populated and `verify-lab-05a.sh` passing —
  `FAIL=0` on support Path A, `FAIL=3` on Path B.
- **Lab 04B and 04C**, for the comparisons in Steps 15, 16 and 17. Nothing from them is consumed
...
The same set as Lab 05A. Nothing new is required, though two optional things help:
...
Everything in Lab 05A, and specifically: the `spec` and `status` controller loop from its Step 12;
...
From Lab 04C: target tracking, cooldown, and the idea that an autoscaler owns a field you then stop
editing by hand.

### 3.4 Support paths, carried forward

Lab 05A Step 2 recorded a support path in `notes/lab-05a-notes.md`. It still applies:
...
| **C** | Section 8.5's paper path. This lab is harder to do on paper than Part A, because its subject is watching numbers move |
...
- Lab 04A: usms-ecs-cluster, usms-enrolment-svc, /usms/ecs/enrolment
- Lab 04B: usms-enrolment-alb, usms-enrolment-tg, listener HTTP:80, rule prio 10
- Lab 04C: scalable target min 2 max 10; CPU and request-count target tracking;
           one step policy; two scheduled actions
- Lab 05A: usms-eks-cluster (tag _lb_ports_=8081,8082), usms-eks-nodes (min 2/max 4/desired 2),
           usms-eks-cluster-role, usms-eks-node-role, usms-eks-cluster-sg
- Lab 05A: namespace usms — gateway, enrolment, results; 4 ConfigMaps; 1 Secret;
...
- nodeSelector workload=usms   on usms-enrolment, using Lab 05A's node group labels

Required for future labs:
- nothing structural. Lab 06 (S3) consumes this lab's Exercise 5 artefacts and
...
| --- | --- | --- |
| Lab 05A `usms-enrolment` CPU request `50m` | Step 8, the HPA's denominator | The HPA reports `<unknown>` and never scales. This is the single most common HPA failure |
| Lab 05A `_lb_ports_=8081,8082` | Step 15 | No host port is published; the LoadBalancer Service is unreachable from your machine |
| Lab 05A node group labels `workload=usms` | Step 11's `nodeSelector` | Pods stay `Pending` with `didn't match node selector` |
| Lab 05A `usms-gateway` Service | Steps 13 to 16 | The whole exposure ladder is climbed on this one object |
| Lab 04B `usms-enrolment-tg` target type `ip` | Step 16's comparison | You lose the connection that makes Ingress on EKS make sense |
| Lab 04C's scalable target | Steps 8 and 17 | You lose the comparison this lab is built around |

**The sentence to say out loud, at Step 16.** Lab 04B created an ALB with a target group of type
...
05A's work. Every object in this lab either modifies something Lab 05A created or sits in front of it.
...
```text
  ...
  browser / curl  ---> localhost:8081  ------+
                                             |   published because Lab 05A tagged the
  ...
  |     was ClusterIP in Lab 05A, was NodePort at Step 14 -- each rung KEEPS the       |
  |     one below it: a LoadBalancer Service still has a nodePort and a clusterIP      |
  |          |                                                                        |
  |          v                                                                        |
  |   Deployment usms-gateway  1 replica                                              |
  |          |  proxy_pass by DNS name, unchanged from Lab 05A                        |
  |          +-------------------------------+                                        |
  |          v                               v                                        |
  ...
  |     target  cpu 50% of the 50m request set in Lab 05A                             |
  |                                                                                   |
...
This lab adds the following. It creates no new folder — Lab 05A's `manifests/` is the home for all of
it.
...
├── scripts/
│   ├── utilities/
│   │   ├── verify-lab-05b.sh                  # NEW — Section 9
│   │   └── hpa-watch.sh                       # NEW — Step 9
│   └── cleanup/
│       └── lab-05b-cleanup.sh                 # NEW — end of course only
└── outputs/
    └── lab-05b-*.txt / *.json                 # command output, git-ignored
```
...
```bash
cd ~/aws-floci-course
mkdir -p labs/lab-05b-eks-scaling manifests/lab-05b
ls -d manifests/*
```
...
```text
manifests/lab-05a  manifests/lab-05b
```
...
!!! info "Where to run every command in this lab"
    ...
    Lab 05A set your `kubectl` context's default namespace to `usms`, so `-n usms` is omitted below.
    ...
    Shell variables still die with the terminal. Step 19 writes this lab's to `configs/lab-05b.env`.

---

### Step 1 — Start the environment, and check the two things Part A had to leave behind

**Purpose**

Two values from Lab 05A determine whether this lab can be done at all. Checking them takes ninety
...
echo "from configs/lab-05a.env : ${USMS_EKS_LB_PORTS:-EMPTY}"
...
from configs/lab-05a.env : 8081,8082
8081,8082
```
...
- **An empty or absent `cpu` request.** Fix it now; Step 8 depends on it and no later step can work
  around it:
    ...
    Then fix `manifests/lab-05a/20-enrolment.yaml` too, so the repository and the cluster agree. A
...
- **`None` for the tag on Path A.** Try `aws eks tag-resource` as Lab 05A Step 9 showed. If that
  fails, Step 15 will be a Conceptual step for you rather than an observable one. Record that in
  `notes/lab-05b-notes.md` now, so your report is accurate rather than apologetic.
...
```bash
./scripts/utilities/verify-lab-05a.sh | tail -3
...
Lab 05A Exercise 5 asked you to record a CPU baseline. This step reads it back and captures a fresh
...
echo "--- Lab 05A Exercise 5 artefacts ---"
ls -l outputs/lab-05a-cpu-baseline.txt outputs/lab-05a-lab06-readiness.txt 2>/dev/null \
  || echo "Exercise 5 not done — that is recoverable, but do it before Lab 06"

echo
echo "--- fresh baseline ---"
{
  echo "=== LAB 05B BASELINE $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
...
} | tee outputs/lab-05b-baseline.txt
```
...
```text
=== LAB 05B BASELINE 2026-09-06T03:40:11Z ===
NAME             DESIRED   READY
usms-enrolment   2         2
usms-gateway     1         1
usms-results     2         2
...
**What to look for:** three Deployments at their Lab 05A counts, every Service `ClusterIP` with
...
```text
Baseline recorded in outputs/lab-05b-baseline.txt
...
In `notes/lab-05b-notes.md`, answer in three or four sentences: *`usms-enrolment` needs to handle ten
...
becomes **ready** — the readiness probe from Lab 05A Step 13 is what gates that, and a pod that is
...
05A Step 16, and the spreading happens beneath it. That is the payoff for the architecture, and it is
...
Kubernetes equivalent of the ALB's deregistration delay that Lab 04B set to 30 seconds.
...
grep -n 'replicas:' manifests/lab-05a/20-enrolment.yaml
```
...
```bash
sed -i.bak 's/^  replicas: 2$/  replicas: 3/' manifests/lab-05a/20-enrolment.yaml

kubectl diff -f manifests/lab-05a/20-enrolment.yaml || true
...
```bash
sed -i.bak 's/^  replicas: 3$/  replicas: 2/' manifests/lab-05a/20-enrolment.yaml
kubectl apply -f manifests/lab-05a/20-enrolment.yaml
kubectl rollout status deployment/usms-enrolment --timeout=120s
...
Scaling, by hand and by file
 ...
 ├── scale-in removes from endpoints BEFORE SIGTERM  (cf. Lab 04B deregistration delay 30s)
...
Lab 05A's containers request `50m`/`32Mi` and limit `200m`/`128Mi`, so all three microservices are
...
second pipeline — Prometheus with an adapter, or on EKS the CloudWatch adapter. Lab 04C scaled on
...
```bash
echo "metrics API unavailable — Step 9 will use the manual fallback" >> notes/lab-05b-notes.md
```
...
correspondence to something you built in Lab 04C, and the comparison is worth more than the object.
...
**The correspondence with Lab 04C, field by field**

| Lab 04C, Application Auto Scaling | Here | Note |
| --- | --- | --- |
...
```bash
cat > manifests/lab-05b/10-hpa-enrolment.yaml << 'EOF'
...
kubectl apply -f manifests/lab-05b/10-hpa-enrolment.yaml
```
...
it is exactly the policy Lab 04C expressed with `ScaleInCooldown: 300` and a shorter scale-out
...
`manifests/lab-05a/20-enrolment.yaml` is now only an *initial* value, and re-applying that file no
...
```bash
cat > manifests/lab-05b/20-loadgen.yaml << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: usms-loadgen
...
kubectl apply -f manifests/lab-05b/20-loadgen.yaml
...
!!! note "Floci Limitation — the metrics pipeline may be absent, and then this step cannot run"
    ...
    **The fallback, and it is a legitimate result.** Do this instead, and say in your report that you
    ...
    Then, in `notes/lab-05b-notes.md`: quote the `ScalingActive: False` condition and its reason
...
aws eks describe-nodegroup \
  ...
  --output json | tee outputs/lab-05b-nodegroup-before.json
```
...
aws eks describe-nodegroup \
  ...
  --query 'nodegroup.scalingConfig' --output json | tee outputs/lab-05b-nodegroup-after.json
...
Leaving it at 3 is fine and is what `configs/lab-05b.env` will record. If you would rather restore
Lab 05A's numbers:
...
Whichever you choose, make `configs/lab-05b.env` in Step 19 record what is actually true.
...
!!! note "Conceptual / Real AWS — neither autoscaler is installed in this laboratory"
    ...
    practice through IRSA, which Lab 05A Step 21 established is not available on this build. Cluster
...
Lab 05A's node group declared Kubernetes labels `workload=usms` and `tier=app`. Those labels are how
...
Two things to notice, and both are the deployment strategy from Lab 05A Step 13 paying off. The new
...
python3 - << 'PY'
import re
p = 'manifests/lab-05a/20-enrolment.yaml'
s = open(p).read()
...
```bash
cat > manifests/lab-05b/30-pdb-gateway.yaml << 'EOF'
...
kubectl apply -f manifests/lab-05b/30-pdb-gateway.yaml
kubectl get pdb
```
...
cat > outputs/lab-05b-eviction.json << EOF
{
  "apiVersion": "policy/v1",
  "kind": "Eviction",
...
kubectl create --raw "/api/v1/namespaces/usms/pods/$POD/eviction" \
  -f outputs/lab-05b-eviction.json 2>&1 | tail -3
```
...
The bottom of the exposure ladder. Everything Lab 05A built is here already; this step names what
...
```bash
cat > manifests/lab-05b/40-service-nodeport.yaml << 'EOF'
apiVersion: v1
kind: Service
metadata:
...
kubectl apply -f manifests/lab-05b/40-service-nodeport.yaml
...
The rung that, on a real cloud, provisions infrastructure. This is where Lab 05A's `_lb_ports_` tag
earns its keep, and where the comparison with Lab 04B begins.
...
```bash
cat > manifests/lab-05b/50-service-loadbalancer.yaml << 'EOF'
apiVersion: v1
kind: Service
metadata:
  name: usms-gateway
...
kubectl apply -f manifests/lab-05b/50-service-loadbalancer.yaml
kubectl get svc usms-gateway -o wide
```

**What the command does**

This is a **complete Service manifest** that replaces the one Lab 05A applied, not a patch. The name
...
aws-load-balancer-nlb-target-type: ip
    ...
    THIS IS LAB 04B's TARGET GROUP TARGET TYPE, IDENTICAL, for the same reason:
...
aws-load-balancer-scheme: internet-facing
    ...
    Lab 04B required both. An internal scheme would use the private pair instead.
```
...
strings. That is the portability claim from Lab 05A Section 1, with its honest caveat attached.
...
```bash
echo "--- via the published host port from Lab 05A's _lb_ports_ tag ---"
...
!!! note "Floci Limitation — a LoadBalancer Service may stay `<pending>` forever, and that is the honest result"
    ...
    `_lb_ports_` tag from Lab 05A Step 8 is what publishes 8081 and 8082 on your host when it does.
    ...
    **Take away the shape.** One Service, one load balancer, one bill. And note the annotation that
    said `target-type: ip` — you configured exactly that by hand in Lab 04B, on an ALB, for a Fargate
...
again next to Lab 04B, where you created an ALB, a target group of type `ip`, a listener, and a
path-based rule — by hand, one AWS call at a time.

```text
LAB 04B, by hand                          LAB 05B, from one Ingress object
...
```bash
cat > manifests/lab-05b/60-ingress.yaml << 'EOF'
...
metadata:
  ...
  annotations:
    ...
    # Each line maps to one command you ran by hand in Lab 04B.
    alb.ingress.kubernetes.io/scheme: internet-facing
    alb.ingress.kubernetes.io/target-type: ip
    alb.ingress.kubernetes.io/listen-ports: '[{"HTTP": 80}]'
    alb.ingress.kubernetes.io/healthcheck-path: /healthz
    alb.ingress.kubernetes.io/tags: Project=USMS,Tier=web,Lab=05B
spec:
  rules:
    - http:
        paths:
...
kubectl apply -f manifests/lab-05b/60-ingress.yaml
kubectl get ingress
...
match. Compare that with Lab 04B, where an ALB listener evaluated rules in **numeric priority order**
...
!!! note "Floci Limitation — an Ingress needs a controller, and one may not be running"
    ...
    **What to take away regardless:** an Ingress is a *declaration of routing intent*. Its value does
    ...
    `aws elbv2` commands from Lab 04B would the AWS Load Balancer Controller have run on your behalf,
    and in what order?* You have all four in Lab 04B's Appendix A.

**Checkpoint 6**

```text
Exposure ladder, complete
 ├── ClusterIP     internal only                                     Lab 05A
 ...
 └── Ingress       usms-portal-ingress   / -> gateway
                                         /enrolment -> enrolment
                                         /results   -> results
                     annotations map 1:1 onto Lab 04B's elbv2 calls
...
Both stacks are up. The ECS service from Lab 04A is still running behind the ALB from Lab 04B with
the scaling policies from Lab 04C, and the EKS cluster from Lab 05A is running the same application
...
```bash
cat >> notes/lab-05b-notes.md << 'EOF'
...
| Question | ECS + ALB (Lab 04B) | Kubernetes + Ingress (Step 16) |
| --- | --- | --- |
...
echo "now fill it in — notes/lab-05b-notes.md"
```
...
} | tee outputs/lab-05b-before-restart.txt
```
...
```bash
{
  ...
  aws eks describe-nodegroup --cluster-name "$USMS_EKS_CLUSTER" --nodegroup-name "$USMS_EKS_NODEGROUP" \
    --query 'nodegroup.scalingConfig' --output text 2>/dev/null || echo "nodegroup: Path B"
} | tee outputs/lab-05b-after-restart.txt

echo
diff <(grep -v '^===' outputs/lab-05b-before-restart.txt) \
     <(grep -v '^===' outputs/lab-05b-after-restart.txt) \
...
### Step 19 — Write `configs/lab-05b.env`
...
export USMS_K8S_MANIFEST_DIR_05B=manifests/lab-05b
EOF

grep -n 'export .*=$\|None' configs/lab-05b.env || echo "all values populated"
```
...
grep -c '^export' configs/lab-05b.env
```
...
- No path under `outputs/` appears. This lab wrote six files there, including
  `lab-05b-eviction.json`, which names a pod and is harmless, and the before/after snapshots.
- No `.env` at the repository root appears.
- `configs/lab-05b.env` **does** appear — names and numbers, no secrets.
- `manifests/lab-05a/20-enrolment.yaml` appears, because Step 11 added a `nodeSelector` to it. That
...
```bash
git check-ignore -v outputs/lab-05b-baseline.txt
```

**Expected result**

```text
.gitignore:7:outputs/*	outputs/lab-05b-baseline.txt
```
...
```bash
git add labs/lab-05b-eks-scaling manifests/lab-05b manifests/lab-05a/20-enrolment.yaml \
        configs/lab-05b.env scripts/utilities/verify-lab-05b.sh \
        scripts/utilities/hpa-watch.sh scripts/cleanup/lab-05b-cleanup.sh \
        notes/lab-05b-notes.md

git status --short

git commit -m "Lab 05B: EKS scaling and service exposure
...
- Ingress annotations mapped one-to-one onto Lab 04B's elbv2 calls
...
**What to look for:** this commit and Lab 05A's beneath it; a file list containing
`manifests/lab-05b/` and `configs/lab-05b.env`; and **`0`** from the last command.

**Checkpoint 8**

```text
committed
 ├── manifests/lab-05b/    10-hpa-enrolment, 20-loadgen, 30-pdb-gateway,
 │                         40-service-nodeport, 50-service-loadbalancer, 60-ingress
 ├── manifests/lab-05a/20-enrolment.yaml   updated with the nodeSelector
 ├── configs/lab-05b.env   19 exports
 ├── scripts/utilities/    verify-lab-05b.sh, hpa-watch.sh
 ├── scripts/cleanup/      lab-05b-cleanup.sh  (written, NOT run)
...
This lab is harder to do on paper than Part A, because its subject is watching numbers move. It is
still worth doing, and here is the honest version:

1. **Write every manifest** in Section 7's list and validate them with
   `kubectl apply --dry-run=client -f manifests/lab-05b/`. No cluster is needed.
...
4. **Do Step 17 in full.** It is the most valuable step in the lab and it requires only that you can
   read Lab 04B and Lab 04C, both of which you have.
...
### 9.1 Build `scripts/utilities/verify-lab-05b.sh`
...
````markdown
{% raw %}```bash
cat > scripts/utilities/verify-lab-05b.sh << 'EOF'
#!/usr/bin/env bash
# Verify every Lab 05B artefact. Exit 1 if anything is missing.
...
source "$REPO_ROOT/configs/lab-05b.env" 2>/dev/null || true
NS=usms
...
echo "== Lab 05B: disruption budgets =="
...
echo "== Lab 05B: data plane (expect FAIL on Path B) =="
...
check "configs/lab-05b.env exists" "test -f configs/lab-05b.env"
check "configs/lab-05b.env records the HPA bounds" \
  "grep -qE '^export USMS_HPA_MIN=[0-9]+' configs/lab-05b.env"
check "six or more Lab 05B manifests present" \
  "test \"\$(ls manifests/lab-05b/*.yaml 2>/dev/null | wc -l)\" -ge 6"
check "every Lab 05B manifest passes a client-side dry run" \
  "kubectl apply --dry-run=client -f manifests/lab-05b/"
...
chmod +x scripts/utilities/verify-lab-05b.sh
./scripts/utilities/verify-lab-05b.sh
```{% endraw %}
````
...
== Lab 05A dependencies ==
  ok   namespace usms exists
  ...
== Lab 05B: exposure ladder ==
...
!!! danger "This script removes everything Lab 05B added"
    **What will be deleted:** the HPA, both PDBs, the NodePort Service, the Ingress and the load
    generator; `usms-gateway` reverts to `ClusterIP`; the node group returns to Lab 05A's numbers.
    **What depends on it:** nothing in Lab 06 or Lab 07. The cluster and the three microservices from
    Lab 05A survive.
    **Reversible?** Yes — every object is in `manifests/lab-05b/` and can be re-applied.
    **Effect on later labs:** none. Run `lab-05b-cleanup.sh` **before** `lab-05a-cleanup.sh`, because
    a LoadBalancer Service can hold a load balancer that blocks the cluster's deletion.

```bash
cat > scripts/cleanup/lab-05b-cleanup.sh << 'EOF'
#!/usr/bin/env bash
# END OF COURSE ONLY. Removes every Lab 05B object. Run BEFORE lab-05a-cleanup.sh.
set -uo pipefail
...
printf 'Type exactly DELETE-LAB-05B to proceed: '
read -r CONFIRM
[ "$CONFIRM" = "DELETE-LAB-05B" ] || { echo "aborted"; exit 1; }
...
echo "Lab 05B objects removed. The Lab 05A cluster and application are untouched."
EOF

chmod +x scripts/cleanup/lab-05b-cleanup.sh
bash -n scripts/cleanup/lab-05b-cleanup.sh && echo "cleanup script: valid bash syntax"
```

The deletion order is the lesson again, and it is different from Lab 05A's: **Ingress, LoadBalancer
...
| --- | --- | --- |
| 1 | Step 2 | Floci running; five env files sourced; `verify-lab-05a.sh` at `FAIL=0` (Path A) or `FAIL=3` (Path B); `usms-enrolment` declaring a CPU request; the `_lb_ports_` tag present or its absence recorded; a baseline in `outputs/lab-05b-baseline.txt` |
...
| 8 | Step 20 | `configs/lab-05b.env` with 19 exports; `manifests/lab-05a/20-enrolment.yaml` updated with the `nodeSelector` so the repository matches the cluster; nothing under `outputs/` staged; `git check-ignore -v` naming the rule that protected you |
...
??? danger "`kubectl scale` appears to work and then the replica count changes back"
    ...
    Note the consequence for your repository: `replicas:` in `manifests/lab-05a/20-enrolment.yaml` is
...
??? danger "`nodePort: 30080` is rejected with `provided port is already allocated`"
    ...
    `manifests/lab-05b/40-service-nodeport.yaml` and `configs/lab-05b.env`. Do not simply remove the
...
| Scheduled scaling (Lab 04C had it for ECS) | CronJob patching an HPA, or KEDA | Not installed | Conceptual / Real AWS |
...
Load Balancer Controller would act on, mapped every annotation to the Lab 04B command it replaces, and
recorded that neither controller is installed on this build" is true, and is worth more.

---
## 13. Independent Lab Exercises

Work in `labs/lab-05b-eks-scaling/exercises.md`. Record commands and output there, and screenshots in
...
- `autoscaling/v2`, written as a manifest at `manifests/lab-05b/11-hpa-results.yaml`. Do not use
  `kubectl autoscale`.
...
`manifests/lab-05b/10-hpa-enrolment.yaml` is the model. The only fields that change are the target
name, the two bounds, the percentage and three numbers inside `behavior`. Check that `usms-results`
actually declares a CPU request before you start — Lab 05A Step 14 set one, but check rather than
assume.
...
- Update `manifests/lab-05a/40-gateway.yaml` so the repository matches the cluster. Show
...
The PDB is not the thing to change. Lab 05A Exercise 4 named three Kubernetes features for spreading
...
### Exercise 5 — Integration: the artefacts Lab 06 needs, and the argument this practical was for

**Requirements**

Three parts.

**Part 1 — capture the scaling evidence.** Produce `outputs/lab-05b-scaling-evidence.json`: a single
...
**Part 2 — verify Lab 06's readiness, from the cluster.** Produce
`outputs/lab-05b-lab06-readiness.txt` showing: the ARN of `USMSStudentDataReadWrite`; every role
...
**Part 3 — write the argument.** Append to `notes/lab-05b-notes.md` a section of no more than 400
...
- Part 2 must read the bucket ARN **out of the policy document**. Typing it means the exercise is not
  done. Lab 05A Exercise 5 did the first half of this; do not copy the answer, re-derive it.
...
notes. Lab 06's first step reads all three.
...
For Part 2's new half: Lab 05A Step 21 laid out the six-step IRSA chain and created
...
- [ ] Five env files sourced, including `configs/lab-05a.env`
- [ ] `verify-lab-05a.sh` run before starting; result recorded
...
- [ ] Baseline captured to `outputs/lab-05b-baseline.txt`
...
- [ ] Every `behavior` field mapped to its Lab 04C equivalent
...
- [ ] `manifests/lab-05a/20-enrolment.yaml` updated so the repository matches the cluster
...
- [ ] Longest-prefix matching distinguished from Lab 04B's numeric rule priority
- [ ] Each `alb.ingress.kubernetes.io` annotation mapped to a Lab 04B `elbv2` command

**Verification, persistence and hygiene**

- [ ] `verify-lab-05b.sh` written and run; result recorded, including expected Path B failures
...
- [ ] `configs/lab-05b.env` generated, 19 exports
...
- [ ] `scripts/cleanup/lab-05b-cleanup.sh` written, syntax-checked, **not run**

**Understanding**

- [ ] Section 15 answered in prose in `notes/lab-05b-notes.md`
...
Answer in prose in `notes/lab-05b-notes.md`. No command output — these ask what you understood.
...
**3.** Lab 04C's Application Auto Scaling and this lab's HorizontalPodAutoscaler both write a single
...
the cost of keeping capacity you no longer need is small change. Lab 04C encoded the same asymmetry as
...
║ configs/lab-05a.env, configs/lab-05b.env ║
...
rm -f manifests/lab-05a/*.bak manifests/lab-05b/*.bak
...
!!! danger "Read before running any delete command"
    ...
    **What depends on them:** nothing. Every one of them was created in this lab as a temporary
    instrument.
    **Reversible?** Yes — `usms-loadgen` is in `manifests/lab-05b/20-loadgen.yaml` and can be
    re-applied at any time.
    **Effect on later labs:** none.

Do **not** run `scripts/cleanup/lab-05b-cleanup.sh`, `lab-05a-cleanup.sh`, `lab-04a-cleanup.sh`,
...
```text
Lab 01  IAM
  usms-developer-role .................. used in Labs 02, 04A and 05A
...
Lab 04A-C  CONTAINERS, the ECS way — still running, not replaced
...
Lab 05A  CONTAINERS, the Kubernetes way
...
Lab 05B  SCALING AND EXPOSURE                              <-- you are here
  SCALING
    usms-enrolment-hpa    autoscaling/v2   min 2  max 8   cpu 50% of the 50m request
  ...
  SAFETY
    ...
    usms-enrolment-pdb    maxUnavailable 1  -> ALLOWED DISRUPTIONS 1
  EXPOSURE
    ClusterIP        the three Lab 05A Services, unroutable from the host
    ...
    usms-portal-ingress   / -> gateway, /enrolment -> enrolment, /results -> results
                     ...
                     healthcheck-path, tags — one per Lab 04B elbv2 call
  and NOTHING from Lab 05A was replaced: same cluster, same images, same ConfigMaps
...
Lab 07  Lambda, then DynamoDB · RDS · SNS/SQS · CloudWatch · CloudFormation
  the CloudWatch lab reads /usms/ecs/enrolment, USMS/Enrolment and Lab 04C's alarms
  the CloudFormation lab re-declares Practical 2 and Practical 4 as one template
```
...
Lab 06 — S3 — creates `usms-student-data`, and it is the moment a policy written in the first
...
| From this lab | Lab 06 uses it for |
| --- | --- |
| `outputs/lab-05b-scaling-evidence.json` (Exercise 5) | Its first `put-object` needs a real file with real content. This is one, and it is one you generated rather than typed |
| `outputs/lab-05b-lab06-readiness.txt` (Exercise 5) | The bucket ARN read out of Lab 01's policy document, the three roles carrying it, and the ServiceAccount that would use it under IRSA |

| From earlier labs | Lab 06 uses it for |
| --- | --- |
| Lab 01 `USMSStudentDataReadWrite` | The policy whose `Resource` names the bucket. Lab 06 creates the bucket the policy already describes, in that order, deliberately |
| Lab 01 `usms-ec2-app-role` | Holder one — an EC2 instance profile, since Lab 03 |
| Lab 04A `usms-ecs-task-role` | Holder two — a Fargate task role, since Practical 2 |
| Lab 05A `usms-eks-node-role` | Holder three — an EKS node role, since Lab 05A Step 7 |
...
| Lab 05A `usms-enrolment-sa` | The ServiceAccount that, on real EKS, would carry the IRSA annotation instead of relying on the node role |

**The connection to state out loud before the next session.** Lab 01 wrote a policy for a bucket that
did not exist. Lab 03 attached it to a virtual machine. Lab 04A attached it, unchanged, to a
serverless container. Lab 05A attached it, unchanged again, to a Kubernetes node. Lab 02 built a VPC
...
**Five laboratories have prepared for one `create-bucket` call**, and when Lab 06 makes it, four
...
./scripts/utilities/verify-lab-05b.sh | tail -2
grep -c '^export' configs/lab-05b.env
...
03, Lab 04A or Lab 05A is missing, and Lab 06's central demonstration loses a third of its force. It
is a one-command fix today.

**Read ahead, five minutes:** find out what an S3 **bucket policy** is and how it differs from the
identity policy you have been attaching to roles since Lab 1. Lab 06 assumes neither, but the
...
```bash
floci snapshot save lab-05b-complete
```
...
```bash
./scripts/setup/floci-down.sh
tar -czf ~/floci-data-lab-05b.tar.gz -C ~ floci-data
./scripts/setup/floci-up.sh
```
...
| `./scripts/utilities/verify-lab-05a.sh` | Lab 05A's 30 checks — run it before this lab |
| `./scripts/utilities/verify-lab-05b.sh` | This lab's 34 checks |
...
| `alb.ingress.kubernetes.io/*` | One annotation per Lab 04B `elbv2` argument | Step 16 |
