...
!!! tip "How to read the probe results"
    ...
    * `UNKNOWN` — inspect the message by hand and reclassify.
    ...
    Three of the probes above are **unconditional creates** — they carry no bogus resource id for the emulator to reject, so if this build implements them, they leave a real object behind. Clean those up now, before they contaminate later labs or your final `describe` counts:

    ```bash
    # a) the probe prefix list
    ...
    # b) the probe DHCP option set (identified by its deliberately invalid domain name)
    while read -r dopt; do
      [ -z "$dopt" ] || [ "$dopt" = "None" ] && continue
      aws ec2 delete-dhcp-options --dhcp-options-id "$dopt" 2>&1 | head -1
    done < <(aws ec2 describe-dhcp-options \
               --filters Name=key,Values=domain-name Name=value,Values=probe.invalid \
               --query 'DhcpOptions[].DhcpOptionsId' --output text 2>/dev/null | tr '\t' '\n')

    # c) a transit gateway, if create-transit-gateway unexpectedly succeeded.
    #    A TGW is the most expensive accidental object in this list — in real AWS it
    #    bills per attachment-hour from the moment it exists.
    while read -r tgw; do
      [ -z "$tgw" ] || [ "$tgw" = "None" ] && continue
      aws ec2 delete-transit-gateway --transit-gateway-id "$tgw" 2>&1 | head -1
    done < <(aws ec2 describe-transit-gateways \
               --query 'TransitGateways[?State!=`deleted`].TransitGatewayId' \
               --output text 2>/dev/null | tr '\t' '\n')
    ```

    !!! warning "This is a general rule about probing, not a detail about three commands"
        Any support probe that does not pass a deliberately non-existent id **is a create**. Before you add an operation to `probe-vpc-support.sh`, ask which of the two kinds it is, and if it is a create, write its teardown in the same commit. `bin/cleanup.sh` in §16 sweeps these again as a backstop, but a probe that cleans up after itself is the correct pattern.
...
```
  ...
  3. Delete flow logs                    do NOT block deletion — that is why they are forgotten
  4. Delete VPC endpoints                interface endpoints own ENIs in subnets
  5. Delete NAT gateways                 own ENIs; their routes become blackholes
  6. Release Elastic IPs                 not released by NAT/instance deletion — BILLED
  7. Delete peering connections          routes referencing them go blackhole
  8. Detach + delete the internet gateway  detach fails while any public IP exists
  9. Restore NACL associations, delete custom NACLs   no disassociate exists
 10. Revoke ALL security group rules, then delete groups   cross-references block deletion
 11. Disassociate + delete route tables  main table cannot be deleted
 12. Delete subnets                      fail while ANY ENI remains
 13. Disassociate secondary CIDRs        primary cannot be disassociated
 14. Delete key pairs, prefix lists, probe artefacts   independent, but tidy
 15. Delete the VPC                      succeeds only when all of the above are done
 16. Sweep by tag                        catch anything created in challenges

Steps 3 and 14 are the ones students forget, and they share a cause: neither
object blocks anything, so nothing forces you to notice it. In AWS those are
precisely the objects that appear on next month's invoice.
```
...
# Section 10.3 creates a flow log on the lab VPC *if* this build supports them.
# Flow logs do not block VPC deletion, which is exactly why they get forgotten —
# in real AWS they keep writing to CloudWatch Logs or S3, and keep billing, long
# after the VPC is gone. Delete by resource id, then sweep anything left.
FLIDS=""
for vpc in $VPCS; do
  ids=$(aws ec2 describe-flow-logs --filter "Name=resource-id,Values=$vpc" \
          --query 'FlowLogs[].FlowLogId' --output text 2>/dev/null)
  [ -n "$ids" ] && [ "$ids" != "None" ] && FLIDS="$FLIDS $ids"
done
# also catch flow logs attached to subnets or ENIs rather than the VPC itself
extra=$(aws ec2 describe-flow-logs \
          --query 'FlowLogs[?starts_with(LogGroupName, `/dnb/`)].FlowLogId' \
          --output text 2>/dev/null)
[ -n "$extra" ] && [ "$extra" != "None" ] && FLIDS="$FLIDS $extra"
FLIDS=$(printf '%s\n' $FLIDS | sort -u | tr '\n' ' ')
if [ -n "${FLIDS// /}" ]; then
  try aws ec2 delete-flow-logs --flow-log-ids $FLIDS
else
  echo "  none (or flow logs unsupported in this build)"
fi
...
step "5. delete NAT gateways  (stops the hourly charge)"
for vpc in $VPCS; do
  while read -r nat; do
    [ -z "$nat" ] && continue
...
step "6. release Elastic IPs  (stops the idle-address charge)"
...
step "7. delete VPC peering connections"
while read -r pcx; do
  [ -z "$pcx" ] && continue
...
step "8. detach and delete internet gateways / egress-only IGWs"
for vpc in $VPCS; do
  while read -r igw; do
    [ -z "$igw" ] && continue
...
step "9. restore default NACL associations, then delete custom NACLs"
for vpc in $VPCS; do
...
step "10. revoke ALL security group rules, then delete non-default groups"
for vpc in $VPCS; do
...
step "11. disassociate and delete non-main route tables"
for vpc in $VPCS; do
  while read -r rtb; do
    [ -z "$rtb" ] && continue
    while read -r assoc; do
...
step "13. disassociate secondary CIDR blocks"
for vpc in $VPCS; do
...
step "14. delete key pairs, prefix lists, probe DHCP option sets and TGWs"
...
while read -r pl; do
  ...
  try aws ec2 delete-managed-prefix-list --prefix-list-id "$pl"
           ...
           --output text 2>/dev/null | tr '\t' '\n')
# Backstop for the unconditional-create probes in Section 0.5.1, in case the
# per-probe teardown there was skipped. Neither object is tagged, so identify by value.
while read -r dopt; do
  [ -z "$dopt" ] || [ "$dopt" = "None" ] && continue
  try aws ec2 delete-dhcp-options --dhcp-options-id "$dopt"
done < <(aws ec2 describe-dhcp-options \
           --filters Name=key,Values=domain-name Name=value,Values=probe.invalid \
           --query 'DhcpOptions[].DhcpOptionsId' --output text 2>/dev/null | tr '\t' '\n')
...
while read -r tgw; do
  [ -z "$tgw" ] || [ "$tgw" = "None" ] && continue
  try aws ec2 delete-transit-gateway --transit-gateway-id "$tgw"
done < <(aws ec2 describe-transit-gateways \
           --query 'TransitGateways[?State!=`deleted`].TransitGatewayId' \
           --output text 2>/dev/null | tr '\t' '\n')
...
step "16. final sweep — anything left tagged Project=CoreBanking or named dnb-*"
